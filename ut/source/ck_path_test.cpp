...
void ck_path_test::test_break_path()
{
    ck_string dir;
    ck_string fileName;

#ifdef IS_WINDOWS
    ck_string p1 = "\\dirA\\dirB\\dirC\\file";
    ck_path::break_path( p1, dir, fileName );
    UT_ASSERT( dir == "\\dirA\\dirB\\dirC" );
    UT_ASSERT( fileName == "file" );
#else
    ck_string p1 = "/dirA/dirB/dirC/file";
    ck_path::break_path( p1, dir, fileName );
    UT_ASSERT( dir == "/dirA/dirB/dirC" );
...
#ifdef IS_WINDOWS
    ck_string p2 = "\\dirA\\dirB\\dirC\\file\\";
    ck_path::break_path( p2, dir, fileName );
    UT_ASSERT( dir == "\\dirA\\dirB\\dirC" );
    UT_ASSERT( fileName == "file" );
#else
    ck_string p2 = "/dirA/dirB/dirC/file/";
...
#ifdef IS_WINDOWS
    ck_string p3 = "\\file";
    ck_path::break_path( p3, dir, fileName );
    UT_ASSERT( dir == "" );
    UT_ASSERT( fileName == "file" );
#else
    ck_string p3 = "/file";
...
void ck_path_test::test_temp_file_name()
{
    MKDIR( "temp_file_name_root" );

    ck_string p1 = ck_path::temp_file_name( "temp_file_name_root", "base" );
    ck_string cmp1 = ck_string::format( "temp_file_name_root%sbase", PATH_SLASH );
    UT_ASSERT( p1.starts_with( cmp1 ) );
    UT_ASSERT( p1.length() == 32 );

    ck_string p2 = ck_path::temp_file_name( "", "" );
    ck_string cmp2 = ck_string::format( ".%s", PATH_SLASH );
    UT_ASSERT( p2.starts_with( cmp2 ) );
    UT_ASSERT( p2.length() == 10 );

    RMDIR( "temp_file_name_root" );
}
