...
void ck_md5::update( const uint8_t* src, size_t size )
...
void ck_md5::get( uint8_t* output )
{
    if( !_finalized )
...
ck_string ck_md5::get_as_string()
{
    if( !_finalized )
        CK_THROW(( "Please finalize() your ck_md5 before calling a get() method." ));
...
const uint8_t* ck_md5::_body( const uint8_t* data, size_t size )
{
    const uint8_t* ptr;
